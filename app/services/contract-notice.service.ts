...
export class ContractNoticeService {
    ...
    getAll(id: string){
        return this.http.get( url + 'api/contract_notice/' + id, this.options)
            .map((res:Response) => res.json())
            .catch((error:any) => Observable.throw(error.json().error || 'Server error'));
    }

    getById(idcontract:string , id:string){
        return this.http.get( url + 'api/contract_notice/' + idcontract +'/'+ id , this.options)
            .map((res:Response) => res.json())
            .catch((error:any) => Observable.throw(error.json().error || 'Server error'));
    }
   ...
   create(body:any, id:string): Promise<Contract> {
    ...
    update(body:Contract, id:string): Promise<Contract> {
        return this.http.post(url + 'api/contract_notice/update/' + id + '/' + body._id,body, this.options)
            .toPromise()
            .then(res => res.json().data)
            .catch(this.handleError);
    }
    ...
    delete(idcontract: string, id:string): Promise<void> {
        return this.http.delete(url + 'api/contract_notice/' + idcontract +'/'+ id, this.options)
            .toPromise()
            .then(() => null)
            .catch(this.handleError);
    }
    ...
    publish(idcontract:string,id: string): Promise<Contract> {
        return this.http.post(url + 'api/contract_notice/'+ idcontract + '/publish/' + id,'', this.options)
            .toPromise()
            .then(res => res.json().data)
            .catch(this.handleError);
    }
